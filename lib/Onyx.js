...
// The delay between batching the writes of non-leader tabs to send to the leader tab
const BATCH_SEND_DELAY = 1;
...
// The queue that stores all writes to be communicated to the tab leader
let batchWrites = [];

// The promise of the clear function, saved so that no writes happen while it's executing
let ongoingClear = false;

// The interval of the execution of the communication of the batched writes to the leader tab
...
function set(key, value) {
    if (!ActiveClientManager.isClientTheLeader()) {
        batchWrites.push({type: 'set', key, value});
...
function multiSet(data) {
    if (!ActiveClientManager.isClientTheLeader()) {
        batchWrites.push({type: 'multiSet', data});
        return Promise.resolve();
    }
...
function merge(key, changes) {
    if (!ActiveClientManager.isClientTheLeader()) {
        batchWrites.push({type: 'merge', key, changes});
        return Promise.resolve();
    }
...
function clear(keysToPreserve = []) {
    if (!ActiveClientManager.isClientTheLeader()) {
        batchWrites.push({type: 'clear', keysToPreserve});
        return Promise.resolve();
    }
    ...
    return getAllKeys()
        .then((keys) => {
            ...
            _.each(keys, (key) => {
                ...
                if (!isKeyToPreserve) {
                    ...
                    if (newValue !== oldValue) {
                        ...
                        if (collectionKey && !!keyValuesToResetAsCollection[collectionKey]) {
                            // CHANGE: notify on initial loop to maintain order of updates between clients
                            keyValuesToResetAsCollection[collectionKey] = true;
                            notifyCollectionSubscribersOnNextTick(collectionKey, {key, newValue});
                        } else {
                            notifySubscribersOnNextTick(key, newValue);
                        }
                    }
                }
            ...
            // CHANGE: use clear to remove "leftover" data issue (3)
            return Storage.clear().then(() => Storage.multiSet(defaultKeyValuePairs)).then(() => {
                ongoingClear = false;
            });
...
function executeEvents(type, data) {
    switch (type) {
        case METHOD.CLEAR:
...
function subscribeToEvents() {
    Broadcast.subscribe((message) => {
        if (message.data.type === 'writes') {
            _.each(message.data.queue, ({type, ...rest}) => {
                executeEvents(type, rest);
            });
        } else {
            executeEvents(message.data.type, message.data);
        }
    });
}

function sendBatchQueue() {
    if (batchWrites.length === 0) {
        return;
    }

    Broadcast.sendMessage({type: 'writes', queue: batchWrites});
    batchWrites = [];
}
...
function init({
    ...
    ActiveClientManager.isReady().then(() => {
        ...
        subscribeToEvents();
            ...
            batchInterval = setInterval(sendBatchQueue, BATCH_SEND_DELAY);
        }
    });
