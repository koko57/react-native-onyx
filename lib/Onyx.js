...
// The delay between the process of each set by the tab leader
const PROCESS_SET_DELAY = 100;
...
// The queue that stores all sets to be executed by the tab leader
let setQueue = [];
...
function set(key, value) {
    if (!ActiveClientManager.isClientTheLeader()) {
        Broadcast.sendMessage({type: 'set', key, value});
...
function multiSet(data) {
    if (!ActiveClientManager.isClientTheLeader()) {
        Broadcast.sendMessage({type: 'multiSet', data});
        return Promise.resolve();
    }
...
function clear(keysToPreserve = []) {
    if (!ActiveClientManager.isClientTheLeader()) {
        Broadcast.sendMessage({type: 'clear', keysToPreserve});
        return Promise.resolve();
    }
...
function processSetQueue() {
    ...
    const {type, ...rest} = setQueue.shift();
    switch (type) {
        case 'set':
            set(rest.key, rest.value);
            break;
        case 'multiSet':
            multiSet(rest.key, rest.value);
            break;
        default:
            break;
    }
...
function subscribeToEvents() {
    Broadcast.subscribe((message) => {
        switch (message.data.type) {
            case 'clear':
                clear(message.data.keysToPreserve);
                break;
            case 'set':
                setQueue.push({type: 'set', ...message.data});
                break;
            case 'multiSet':
                setQueue.push({type: 'multiSet', ...message.data});
                break;
            case 'queue':
                setQueue = [...message.data.queue, setQueue];
                break;
            default:
                break;
        }
...
function init({
    ...
    ActiveClientManager.isReady().then(() => {
        ...
        executionInterval = setInterval(processSetQueue, PROCESS_SET_DELAY);
        subscribeToEvents();
    });
    ...
    window.onbeforeunload = () => {
        ...
        clearInterval(executionInterval);
        Broadcast.sendMessage({type: 'queue', queue: setQueue});
    };
